...
func validateHostDriver(silent bool) error {
	...
	command := "chroot"
	args := append([]string{"/host", nvidiaSMIPath}, nvidiaSMIArgs()...)
...
func waitForParentDevices(ctx context.Context, timeout time.Duration) error {
	...
	return wait.PollUntilContextTimeout(ctx, pollInterval, timeout, true, func(ctx context.Context) (bool, error) {
		...
		if isDriverUsingSRIOV(gpus) {
			return AreAllVFsReady(gpus), nil
		}
		return mdevParentDevicesExist(nvmdevLib), nil
	})
}

// isDriverUsingSRIOV reports whether any GPU is an SR-IOV PF with VFs enabled;
// AreAllVFsReady checks completeness across all PFs.
func isDriverUsingSRIOV(gpus []*nvpci.NvidiaPCIDevice) bool {
...
func mdevParentDevicesExist(nvmdevLib nvmdev.Interface) bool {
