...
func DatadogGetEFASpec(pool nodePool) (*efaDriverSpec, error) {
	...
	imagePath, err := nvidiav1alpha1.GetEFAPrecompiledImagePath(pool.osTag, pool.kernel)
	if err != nil {
		return nil, err
	}

	nvPeermemImagePath, err := nvidiav1alpha1.GetEFANVPeermemPrecompiledImagePath(pool.osTag, pool.kernel)
	if err != nil {
		return nil, err
	}
